...
## test/functional: Run functional tests (requires Docker for Kafka)
test/functional:
	go test -v -race -count=1 -tags=functional -timeout=120s ./tests/functional/...

## test/e2e: Run end-to-end tests (requires Docker + binary build)
test/e2e: build
	go test -v -count=1 -tags=e2e -timeout=180s ./tests/e2e/...
