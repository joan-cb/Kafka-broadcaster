...
Service that consumes JSON messages from a single Kafka source topic, applies configurable transformation and enrichment, and routes outbound messages to target topics (via headers or optional content-based rules). Failed produces are sent to a DLQ topic. Prometheus metrics can be exposed on a configurable HTTP port.

A small **Kafka dev UI** (`kafkaui`) is included for local development: list topics, produce messages, and consume streams in the browser.
...
| `broadcaster.dlq` | Dead-letter topic for failed produces (can often be auto-created on first produce if your cluster allows it) |
