...
func skipE2EIfNoDocker(t *testing.T) {
	t.Helper()
	if dockerSocketLikelyPresent() {
		return
	}
	t.Skipf("e2e tests need Docker: no local Docker socket found and DOCKER_HOST is unset or not unix/tcp. Start Docker Desktop (macOS) or the Docker daemon (Linux), then re-run.")
}
